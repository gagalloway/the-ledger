...
/* ---------- home dashboard ---------- */
function BagEditor({ bag, onChange }) {
  const C = useTheme();
  const [list, setList] = useState(() => (bag && bag.length ? bag.map((c) => ({ ...c })) : DEFAULT_BAG.map((c) => ({ ...c }))));

  // save-on-change: any edit auto-commits to parent state
  const commit = (next) => { setList(next); onChange(next.map((c) => ({ ...c, name: c.name.trim() })).filter((c) => c.name)); };
  const toggle = (i, key) => commit(list.map((c, j) => j === i ? { ...c, [key]: !c[key] } : c));
  const rename = (i, name) => { const next = list.map((c, j) => j === i ? { ...c, name } : c); setList(next); onChange(next.map((c) => ({ ...c, name: c.name.trim() })).filter((c) => c.name)); };
  const remove = (i) => commit(list.filter((_, j) => j !== i));
  const add = () => setList((L) => [...L, { name: "", tee: false, appr: true }]);
  const reset = () => commit(DEFAULT_BAG.map((c) => ({ ...c })));

  return (
    <>
      <div style={{ fontSize: 13, color: C.secondary, marginBottom: 14 }}>
        Tap a name to edit. Tag each club for tee, approach, or both. Order here = order in the app.
      </div>
...
function AppearancePanel({ theme, onToggleTheme }) {
  const C = useTheme();
  return (
    <>
      <div style={{ fontSize: 13, color: C.secondary, marginBottom: 14 }}>
        Choose how the app looks. It syncs with your device by default.
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        {[{ k: "light", label: "Light" }, { k: "dark", label: "Dark" }].map(({ k, label }) => (
          <button key={k} onClick={() => { if (theme !== k) onToggleTheme(); }}
            style={{ flex: 1, padding: "14px 12px", borderRadius: 14, border: "none", cursor: "pointer",
              background: theme === k ? C.blue : C.card, color: theme === k ? "#fff" : C.label,
              fontSize: 16, fontWeight: 600, fontFamily: SYS }}>{label}</button>
        ))}
      </div>
    </>
  );
...
function SettingsSheet({ bag, theme, onBagChange, onToggleTheme, data, onImport, onClose }) {
  const C = useTheme();
  const [panel, setPanel] = useState(null); // null | 'appearance' | 'bag' | 'backup'

  const title = panel === "appearance" ? "Appearance"
    : panel === "bag" ? "My Bag"
    : panel === "backup" ? "Backup"
    : "Settings";

  const RowLink = ({ id, icon, label, sub }) => (
    <button onClick={() => setPanel(id)} style={{ width: "100%", display: "flex", alignItems: "center",
      gap: 12, background: C.card, border: "none", borderRadius: 12, padding: "14px 14px",
      marginBottom: 8, cursor: "pointer", textAlign: "left" }}>
      <div style={{ width: 32, height: 32, borderRadius: 8, background: C.fill,
        display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
        {icon}
      </div>
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 16, fontWeight: 600, color: C.label }}>{label}</div>
        {sub && <div style={{ fontSize: 12, color: C.secondary, marginTop: 2 }}>{sub}</div>}
      </div>
      <div style={{ fontSize: 22, color: C.tertiary }}>›</div>
    </button>
  );

  return (
    <div style={{ position: "fixed", inset: 0, zIndex: 70, background: C.bg,
      overflowY: "auto", animation: "ybSlideR .28s cubic-bezier(.2,.9,.3,1)",
      paddingTop: "env(safe-area-inset-top)", paddingBottom: "env(safe-area-inset-bottom)" }}>
      <div style={{ maxWidth: 560, margin: "0 auto", padding: "14px 16px 40px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8, minWidth: 0 }}>
            {panel && (
              <button onClick={() => setPanel(null)} style={{ background: "none", border: "none",
                color: C.blue, fontSize: 17, cursor: "pointer", padding: 0, fontFamily: SYS }}>‹ Settings</button>
            )}
            {!panel && <div style={{ fontSize: 28, fontWeight: 700, color: C.label }}>Settings</div>}
          </div>
          <button onClick={onClose} style={{ ...btn(C.fill, C.label), padding: "9px 14px", fontSize: 15, width: "auto" }}>Done</button>
        </div>

        {panel === null && (
          <>
            <RowLink id="appearance"
              sub={theme === "dark" ? "Dark" : "Light"}
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.label} strokeWidth="1.8" strokeLinecap="round">
                {theme === "dark"
                  ? (<><circle cx="12" cy="12" r="4.5" /><line x1="12" y1="2.5" x2="12" y2="5" /><line x1="12" y1="19" x2="12" y2="21.5" /><line x1="2.5" y1="12" x2="5" y2="12" /><line x1="19" y1="12" x2="21.5" y2="12" /></>)
                  : <path d="M20 13.5A8 8 0 0 1 10.5 4 6.8 6.8 0 1 0 20 13.5Z" />}
              </svg>}
              label="Appearance" />
            <RowLink id="bag"
              sub={`${(bag || DEFAULT_BAG).length} clubs`}
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.label} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
              ...
              label="My Bag" />
            <RowLink id="backup"
              sub={`${data.rounds.length} rounds saved`}
              icon={<svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke={C.label} strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
        ...
        {panel !== null && (
          <div style={{ marginTop: 4 }}>
            <div style={{ fontSize: 22, fontWeight: 700, color: C.label, marginBottom: 12 }}>{title}</div>
            {panel === "appearance" && <AppearancePanel theme={theme} onToggleTheme={onToggleTheme} />}
            {panel === "bag" && <BagEditor bag={bag} onChange={onBagChange} />}
            {panel === "backup" && <Backup data={data} onImport={onImport} bare />}
          </div>
        )}
      </div>
    </div>
...
function Home({ data, stats, onTeeOff, onResume, theme, onToggleTheme, goTrends, onImport, onOpenSettings }) {
  const C = useTheme();
  const s = stats;
  ...
  return (
    <div>
      ...
      <div style={{ textAlign: "center", fontSize: 11, color: C.tertiary, margin: "40px 0 4px" }}>
        {APP_NAME} · {APP_BYLINE}
      </div>
      <div style={{ height: 8 }} />
    </div>
...
/* ---------- backup / restore ---------- */
function Backup({ data, onImport, bare }) {
  const C = useTheme();
...
function TraceLine({ holes, startHole, label, universe, match, tintKey = "blue", last,
  ...
  const hitEntries = uni.filter(({ h }) => match(h));
  const missEntries = uni.filter(({ h }) => !match(h));
  const den = uni.length;
  const hits = hitEntries.map(({ i }) => i);
  const misses = missEntries.map(({ i }) => i);
  const hitNums = hits.map((i) => startHole + i);
  const missNums = misses.map((i) => startHole + i);
  const pctv = den ? Math.round((hits.length / den) * 100) : null;
  const active = activeLabel === label;
  const disabled = den === 0;

  return (
    <div style={{ borderBottom: last ? "none" : `0.5px solid ${C.line}` }}>
      <div onClick={() => !disabled && onTrace(active ? null : { label, hits, misses })}
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
          {!countOnly && pctv != null && (
            <span style={{ fontSize: 13, color: C.tertiary }}>{hits.length}/{den}</span>
          )}
          <span style={{ fontSize: 15, fontWeight: 700, color: tint, minWidth: 40, textAlign: "right" }}>
            {countOnly ? hits.length : (pctv != null ? pctv + "%" : "—")}
          </span>
        </div>
      </div>
      {!countOnly && pctv != null && (
      ...
      {active && (
        <div style={{ fontSize: 13, padding: "0 0 11px", lineHeight: 1.6 }}>
          {hitNums.length > 0 && (
            <div style={{ color: tint }}>
              ✓ Hole{hitNums.length > 1 ? "s" : ""} {hitNums.join(", ")}
            </div>
          )}
          {missNums.length > 0 && (
            <div style={{ color: C.red }}>
              ✕ Hole{missNums.length > 1 ? "s" : ""} {missNums.join(", ")}
            </div>
          )}
          {hitNums.length === 0 && missNums.length === 0 && (
            <div style={{ color: C.secondary }}>No holes</div>
          )}
        </div>
      )}
    </div>
...
function ScoreCell({ hole, num, highlight, dim, miss }) {
  const C = useTheme();
  const h = hole;
  ...
  return (
    <div style={{ textAlign: "center", paddingBottom: 3, position: "relative",
      ...
      opacity: dim && !miss ? 0.32 : 1 }}>
      {miss && (
        <div style={{ position: "absolute", top: 2, right: 2, width: 12, height: 12,
          borderRadius: 6, background: C.red, color: "#fff", fontSize: 9, fontWeight: 800,
          display: "flex", alignItems: "center", justifyContent: "center", lineHeight: 1, zIndex: 2 }}>✕</div>
      )}
...
function RoundDetail({ round: r, rounds, onEdit, onBack, titleText, bag }) {
  const C = useTheme();
  const [trace, setTrace] = useState(null); // { label, hits: [idxs], misses: [idxs] } | null
  const rs = roundStats(r);
  const startHole = r.startHole || 1;
  const pct = (n, d) => (d ? Math.round((n / d) * 100) + "%" : "—");
  const sug = computeSuggestions(computeStats(rounds.slice(-5)));
  const hitSet = trace ? new Set(trace.hits) : null;
  const missSet = trace ? new Set(trace.misses) : null;
  const inScope = trace ? new Set([...trace.hits, ...trace.misses]) : null;
  ...
  const bagOrder = (bag && bag.length ? bag : DEFAULT_BAG).map((c) => c.name);
  const bagSort = (a, b) => {
    const ai = bagOrder.indexOf(a), bi = bagOrder.indexOf(b);
    if (ai === -1 && bi === -1) return a.localeCompare(b);
    if (ai === -1) return 1;
    if (bi === -1) return -1;
    return ai - bi;
  };
  const teeClubsUsed = [...new Set(r.holes.filter((h) => h.par > 3 && h.tee.result && h.tee.club).map((h) => h.tee.club))].sort(bagSort);
  const apprClubsUsed = [...new Set(r.holes.filter((h) => h.app.club).map((h) => h.app.club))].sort(bagSort);
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(9, 1fr)", gap: 4 }}>
          {r.holes.map((h, j) => (
            <ScoreCell key={j} hole={h} num={startHole + j}
              highlight={hitSet ? hitSet.has(j) : false}
              miss={missSet ? missSet.has(j) : false}
              dim={inScope ? !inScope.has(j) : false} />
          ))}
        </div>
        {trace && (
...
function LastRound({ rounds, onEdit, bag }) {
  const C = useTheme();
  ...
  return <RoundDetail round={rounds[rounds.length - 1]} rounds={rounds} bag={bag} onEdit={onEdit} titleText="Last Round" />;
}
    ...
    <ThemeCtx.Provider value={C}>
      <div style={{ minHeight: "100vh", background: C.bg, fontFamily: SYS, color: C.label }}>
        <style>{`button:active{opacity:.7} input::placeholder{color:${C.tertiary}}
          ...
          @keyframes ybSlideR{from{transform:translateX(100%)}to{transform:translateX(0)}}
        ...
        <div style={{ maxWidth: 560, margin: "0 auto", padding: "14px 16px 100px" }}>
          ...
          {tab === "last" && <LastRound rounds={data.rounds} bag={data.bag}
            onEdit={(r) => { editRound(r); }} />}
          {tab === "rounds" && (() => {
            const detail = detailId != null ? data.rounds.find((r) => r.id === detailId) : null;
            if (detail) return (
              <RoundDetail round={detail} rounds={data.rounds} bag={data.bag} titleText="Round"
        ...
        {settingsOpen && <SettingsSheet
          bag={data.bag}
          theme={data.theme}
          onBagChange={(bag) => setData((d) => ({ ...d, bag }))}
          onToggleTheme={() => setData((d) => ({ ...d, theme: d.theme === "dark" ? "light" : "dark" }))}
          data={data}
          onImport={(imp) => setData((d) => ({
            ...d,
            courses: imp.courses || [],
            rounds: (imp.rounds || []).map((r) => ({ ...r, holes: r.holes.map(migrateHole) })),
          }))}
